 ...
 *
 * NOTE: This base hook supports multiple pools. Pool keys are stored in a mapping by their pool ID
 * and are automatically registered when a pool is initialized via the `PoolManager`. The pool key
 * must be passed in the `AddLiquidityParams` and `RemoveLiquidityParams` structs.
 *
...
abstract contract BaseCustomAccounting is BaseHook, IHookEvents, IUnlockCallback {
    ...
    /**
     * @notice Mapping of pool IDs to their pool keys.
     */
     ...
     * @return The pool key associated with the given ID.
     */
    function poolKey(PoolId id) public view returns (PoolKey memory) {
        return _poolKeys[id];
    }

    /**
     * @notice Checks if a pool has been initialized with this hook.
    ...
    function isPoolInitialized(PoolId id) public view returns (bool) {
        return address(_poolKeys[id].hooks) != address(0);
    }

    /**
     * @notice Adds liquidity to a pool managed by this hook.
     *
     ...
     *
     * @param params The parameters for the liquidity addition, including the pool key.
    ...
    /**
     * @notice Removes liquidity from a pool managed by this hook.
     *
     ...
     *
     * @param params The parameters for the liquidity removal, including the pool key.
        ...
        returns (BalanceDelta callerDelta, BalanceDelta feesAccrued)
    {
        (callerDelta, feesAccrued) = abi.decode(
            poolManager.unlock(abi.encode(CallbackData(key, msg.sender, abi.decode(params, (ModifyLiquidityParams))))),
    ...
    /**
     * @dev Initialize a pool's key. Each pool key is stored in a mapping by its pool ID.
     * This allows the hook to work with multiple pools.
     */
    function _beforeInitialize(address, PoolKey calldata key, uint160) internal virtual override returns (bytes4) {
        // Store the pool key in the mapping by its pool ID
        _poolKeys[key.toId()] = key;
